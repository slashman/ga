...
public class Plasma {
	...
	private int calculateHeight(int a, int b, RandomGen rg){
		int h = (int) (avg(a, b) + var * (rg.nextDouble() - 0.5));
...
*/
	...
	public int[][] buildStripPlasma(int MAX, int MIN, int var, int SIZE, long seed, int [] strips){
		...
		for (int i = 0; i<8; i++){
			...
			map [3*(SIZE/4)][(SIZE/8)*i] = strips[i];
		}
	...
	public int[][] buildBoundedPlasma(int MAX, int MIN, int var, int SIZE, long seed, int [] strips){
    ...
    public int [][] buildPlasma(int MAX, int MIN, int var, int SIZE, long seed){
	...
	private int[][] makePlasma(int MAX, int MIN, int var, int SIZE, RandomGen rg){
        int inc = SIZE;
        while (inc > 1){
